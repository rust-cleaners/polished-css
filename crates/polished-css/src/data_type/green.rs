...
mod test {
	...
	fn display() {
		use crate::data_type::*;
		assert_eq!(super::Green::number(0.1).to_string(), String::from("0.1"));
		assert_eq!(
			super::Green::percentage(10.0).to_string(),
			String::from("10%")
		);
		assert_eq!(super::Green::min().to_string(), String::from("0"));
		assert_eq!(super::Green::max().to_string(), String::from("255"));
	}
}
